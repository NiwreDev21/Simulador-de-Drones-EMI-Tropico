...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QFrame, QSizePolicy, QSlider, QGroupBox
)
...
class DroneGUI(QWidget):
    ...
    def init_ui(self):
        ...
        # --- Layout principal de cámaras ---
        cameras_layout = QHBoxLayout()

        # Columna izquierda: Observer (grande)
        ...
        self.label_obs.setMinimumSize(600, 700)
        left_col.addWidget(self.label_obs)

        # Columna derecha: FPV + TPV
        ...
        self.label_fp.setMinimumSize(400, 300)
        ...
        # Control de cámara Gazebo
        cam_group = QGroupBox("Control de Cámara (Yaw)")
        cam_layout = QVBoxLayout()
        self.cam_label = QLabel("Rotación: 0°")
        self.cam_slider = QSlider(Qt.Horizontal)
        self.cam_slider.setMinimum(0)
        self.cam_slider.setMaximum(360)
        self.cam_slider.setValue(0)
        self.cam_slider.valueChanged.connect(self.update_camera)
        cam_layout.addWidget(self.cam_label)
        cam_layout.addWidget(self.cam_slider)
    ...
    def toggle_subs(self):
        if not self.sub_fp:
            ...
            for sub in [self.sub_fp, self.sub_tp, self.sub_obs, self.sub_alt, self.sub_batt, self.sub_speed, self.sub_state]:
                if sub: sub.unregister()
            self.sub_fp = self.sub_tp = self.sub_obs = self.sub_alt = self.sub_batt = None
    ...
    def cb_observer(self, data):
        try: self.image_obs = self.bridge.imgmsg_to_cv2(data, "bgr8")
        except Exception as e: rospy.logwarn(f"Observer Error: {e}")
    ...
    def refresh_images(self):
        ...
        if self.image_obs is not None: self.label_obs.setPixmap(self.cv_to_pixmap_responsive(self.image_obs, self.label_obs))
    ...
    def update_camera(self, value):
        self.cam_label.setText(f"Rotación: {value}°")
        pose = Pose()
        pose.position.x = 5.0
        pose.position.y = 0.0
        pose.position.z = 2.0
        q = tft.quaternion_from_euler(0, 0, math.radians(value))
